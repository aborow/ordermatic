...
class BomStructureXlsx(models.AbstractModel):
		...
		if workcenters:
			if bom:
				...
				for od in operation_details:
					...
					sheet.write(i, 13, duration_minutes)
					sheet.write(i, 14, 'Minutes')
					sheet.write(i, 16, '')
					sheet.write(i, 17, od.bom_cost or '')
					i += 1
		i += 1
	...
	def generate_xlsx_report(self, workbook, data, objects):
		...
		align = workbook.add_format({'align': 'left','bold': True})
		...
		for o in objects:
			...
			if workcenters:
				...
				for od in operation_details:
					duration_minutes = '%02d:%02d' % (int(float(od.quantity)), float(od.quantity) % 1 * 60)
					wc_name = od.operation_id.name + " " + '-' + " " + od.operation_id.workcenter_id.name if od.operation_id.workcenter_id else od.operation_id.name
					sheet.write(i, 0, wc_name   or '', bold)
					sheet.write(i, 13, duration_minutes, bold)
					sheet.write(i, 14, 'Minutes', bold)
					sheet.write(i, 16,'')
					sheet.write(i, 17, od.bom_cost or '', bold)
					i += 1
			i += 1
			j = 0
