...
class AccountInvoice(models.Model):
    ...
    def _add_tracking_numbers(self):
        for invoice in self:
            if invoice.invoice_line_ids:
                ...
                if sale_line_ids:
                    sale_order_recs = self.env['sale.order.line'].browse(
    ...
    def compute_amount_discount(self):
        self.amount_discount = sum(
            line.discount_amount for line in self.invoice_line_ids)
    ...
    def _get_origin_date(self, origin):
        ...
        if sale_order.confirmation_date:
            confirmation_date = datetime.datetime.strptime(
                str(sale_order.confirmation_date), '%Y-%m-%d %H:%M:%S').date()
    ...
    def _get_shipping_date(self, origin):
        ...
        if stock_picking.date_done:
            date_done = datetime.datetime.strptime(
                str(stock_picking.date_done), '%Y-%m-%d %H:%M:%S').date()
            return date_done
        else:
            return
...
class AccountInvoiceLine(models.Model):
    ...
    discount_amount = fields.Float(
        'Discount Amount', compute='compute_discount_amount')
    ...
    def compute_discount_amount(self):
        for invoice in self:
            if invoice.discount:
                ...
                invoice.discount_amount = round(
                    price - invoice.price_subtotal, 2)
            else:
