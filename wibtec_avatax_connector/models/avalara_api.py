...
class AvaTaxService:
    ...
    def script_get_result(self, svc, operation, request):
        try:
            result = operation(request).decode('utf-8')
        except suds.WebFault as e:
            raise UserError(_(e.fault.faultstring))
        except urllib.error.HTTPError as e:
            raise UserError(_(e.code))
    ...
    def script_validate_address(self, baseaddress, textcase='Default'):
        ...
        result = self.script_get_result(self.addressSvc, self.addressSvc.service.Validate, request)
        return result
