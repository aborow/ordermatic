...
class StockCycleCount(models.Model):
    ...
    #to make it easier to use the server action
    @api.multi
    def action_create_inventory_adjustment_multi(self):
        if self._context.get('active_ids'):
            for s in self.browse(self._context['active_ids']):
                s.action_create_inventory_adjustment()
